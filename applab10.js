...
//const { buffer } = require('stream/consumers');// QUE ES ESTO
...
//Arreglos para guardar las plantas 
const plantas = [];
...
const server =  http.createServer( (request, response) => {
  ...
  if(request.method== "GET"&& (Request.url =="agregar"|| request.url == "/")){
    ...
    // on escucha todos los metodos de la petición por que no siempre llega completa
    //data es un evento y ocurre cuando llegan datos al servidor 
    // funcion o un apuntador a una funcion
    //la funcion se hace cuado te mandan data
    ...
    //llego la info en exadecimal 
    request.on("end", () => {
  
      //concatene
      const string_datos_completos=Buffer.concat(datos_completos).toString();
      console.log(string_datos_completos);
      //split es para separar un string por el parametro recibido y 
      // lo pone en un arreglo
      ...
      //para ir añadiendo a un archivo de texto las nuevas plantas
      file_system.appendFileSync("Registro_de_plantas.txt", nueva_planta);
      ...
      for (const planta of plantas) {
        ...
        response.write(`<div class="card">  
          ...
          <div class="content">`);
        //Escritura sincronico bloqueo el sistema escribe nuevo contenido, pero append solo agrega no sobre escribe 
